...
class SimpleTclGeneratorIntegrationTest extends AbstractTclGeneratorIntegrationTest {
	...
	def void testAssertionGeneration() {
		...
		generatedCode.assertContains('''
			...
			public void execute() throws Exception {
			  ...
			  reporter.enter(TestRunReporter.SemanticUnit.STEP, "assert baz = \"fix\" // baz = '" + baz + "'");
			  org.junit.Assert.assertEquals("SimpleTest.tcl:12: baz = \"fix\"", "fix", baz);
			  reporter.enter(TestRunReporter.SemanticUnit.STEP, "assert book // book = '" + book + "'");
			  org.junit.Assert.assertTrue("SimpleTest.tcl:13: book", book);
			  reporter.enter(TestRunReporter.SemanticUnit.STEP, "assert mak.\"key with spaces\" = \"fox\" // mak = '" + mak + "'");
			  ...
			  reporter.enter(TestRunReporter.SemanticUnit.STEP, "assert mak.\"key with spaces\" // mak = '" + mak + "'");
			  ...
			  reporter.enter(TestRunReporter.SemanticUnit.STEP, "assert log != baz // log = '" + log + "', baz = '" + baz + "'");
			  org.junit.Assert.assertNotEquals("SimpleTest.tcl:24: log != baz", Long.parseLong(baz), log);
			  reporter.enter(TestRunReporter.SemanticUnit.STEP, "assert baz != book // baz = '" + baz + "', book = '" + book + "'");
			  org.junit.Assert.assertNotEquals("SimpleTest.tcl:25: baz != book", Boolean.toString(book), baz);
			  reporter.enter(TestRunReporter.SemanticUnit.STEP, "assert mak.\"key\" < \"42\" // mak = '" + mak + "'");
	...
	def void testCoercionAndChecks() {
		...
		generatedCode.assertContains('''
			...
			reporter.enter(TestRunReporter.SemanticUnit.STEP, "Type boolean @stringvar into <Input> // stringvar = '" + stringvar + "'");
			...
			reporter.enter(TestRunReporter.SemanticUnit.STEP, "TypeLong @stringvar into <Input> // stringvar = '" + stringvar + "'");
			...
			reporter.enter(TestRunReporter.SemanticUnit.STEP, "Type boolean @jsonvar.\"key5\" into <Input> // jsonvar = '" + jsonvar + "'");
			...
			reporter.enter(TestRunReporter.SemanticUnit.STEP, "TypeLong @jsonvar.\"key6\" into <Input> // jsonvar = '" + jsonvar + "'");
			...
			reporter.enter(TestRunReporter.SemanticUnit.STEP, "Set enum of <Input> to @enumvar // enumvar = '" + enumvar + "'");
			dummyFixture.setEnum("text.input", enumvar);
			reporter.enter(TestRunReporter.SemanticUnit.STEP, "Set enum of <Input> to @stringvar // stringvar = '" + stringvar + "'");
